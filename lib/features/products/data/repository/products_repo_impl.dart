...
class ProductsRepoImpl extends ProductRepo {
  ...
  Future<Either<Failures, List<Product>>> getProducts() async{
    try{
      final List<ProductModel> response = await _productService.getProducts(token: "token");
      final List<Product> products = response.map((productModel) => productModel.toDomain()).toList();
